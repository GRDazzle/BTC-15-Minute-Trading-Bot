"""Train RL meta-controller: XGB predicts direction, RL decides WHEN to enter.

Usage:
    python scripts/train_rl_meta.py --asset BTC --timesteps 500000
    python scripts/train_rl_meta.py --asset BTC,ETH,SOL,XRP --timesteps 500000
"""
import argparse
import sys
import time
...
def train_asset(asset: str, timesteps: int, eval_split: float = 0.2):
    ...
    from rl.meta_controller_env import MetaControllerEnv
    from rl.trading_env import load_training_windows

    print(f"\n{'='*60}")
    print(f"  RL Meta-Controller: {asset}")
    print(f"  (XGB predicts direction, RL decides timing)")
    print(f"{'='*60}")
    ...
    train_env = DummyVecEnv([lambda: MetaControllerEnv(windows=train_windows)])
    ...
    model = PPO(
        ...
        n_epochs=10,
        gamma=0.99,
        ent_coef=0.02,      # v2: lower entropy — less random, more decisive
        clip_range=0.2,
        device="cpu",
        verbose=0,
    )
    ...
    eval_env = MetaControllerEnv(windows=eval_windows)
    ...
    for w in eval_windows:
        ...
        eval_env._first_price = None
        eval_env._best_price = 999
        ...
        # --- XGB baseline (enter at FIRST qualifying checkpoint) ---
        entered = False
    ...
    print(f"\n  {'':>15} {'Trades':>7} {'WR%':>6} {'PnL':>10} {'Avg Price':>10}")
    print(f"  {'RL meta':>15} {rl_trades:>7} {rl_wr:>5.1f}% ${rl_pnl:>+9.2f} {rl_avg_price:>9.1f}c")
    print(f"  {'XGB baseline':>15} {bl_trades:>7} {bl_wr:>5.1f}% ${bl_pnl:>+9.2f} {bl_avg_price:>9.1f}c")
    print(f"  {'RL skipped':>15} {rl_skips:>7}")
    delta = rl_pnl - bl_pnl
    price_saved = bl_avg_price - rl_avg_price
    print(f"  {'Delta':>15} {'':>7} {'':>6} ${delta:>+9.2f} {price_saved:>+8.1f}c saved")

    model_path = PROJECT_ROOT / "models" / f"{asset}_rl_meta.zip"
    ...
    return {
        ...
        "rl_avg_price": rl_avg_price,
        "bl_pnl": bl_pnl, "bl_trades": bl_trades, "bl_wr": bl_wr,
        "bl_avg_price": bl_avg_price,
        "delta": delta, "price_saved": price_saved,
    }
...
def main():
    ...
    if results:
        print(f"\n{'='*60}")
        print(f"  SUMMARY: RL Meta-Controller vs XGB Baseline")
        print(f"{'='*60}")
        print(f"  {'Asset':<6} {'RL PnL':>9} {'XGB PnL':>9} {'Delta':>9} {'RL WR':>6} {'XGB WR':>6} {'Price saved':>11}")
        for r in results:
            print(f"  {r['asset']:<6} ${r['rl_pnl']:>+8.2f} ${r['bl_pnl']:>+8.2f} ${r['delta']:>+8.2f} {r['rl_wr']:>5.1f}% {r['bl_wr']:>5.1f}% {r['price_saved']:>+9.1f}c")
